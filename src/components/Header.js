...
export default function Header() {
  ...
  return (
    <>
     <header className='absolute z-10 p-8 w-full flex items-center justify-center lg:items-start lg:justify-start'>
        <div>
        ...
        <div className='absolute left-8 top-8 z-20 lg:hidden'>
            {isOpen ? <button onClick={()=>setIsOpen(false)}><img src={close} alt=""/></button>:<button  onClick={()=>setIsOpen(true)}>
                <img src={menu} alt=""/> 
            </button>}
        </div>
     </header>
